...
How to run the pipeline. Everything else — configuration keys, benchmarking,
cluster tuning, worked examples — is in the
...
`init` creates `cosigt_smk/config/` and copies each shipped example to the name
...
Only one of `assemblies.tsv` and `alleles.tsv` is used, depending on
`allele_source`. `truth_graphs.tsv.example` is not copied, because it is only
needed for `benchmark_mode: leave_all_out`; copy it yourself if you need it.

It also writes `.cosigt.mk` with the settings below, so later commands can be
run without repeating them. Existing files are never overwritten, so `init` is
safe to re-run and will not touch edits you have already made.
...
- Finally, it composes the Apptainer flags this run needs, writes them to
  `cosigt_smk/.cosigt/apptainer.args`, and persists the validated settings to
  `.cosigt.mk`.
...
Set `apptainer_extra` in `config.yaml` to append site-specific flags, or
`apptainer_cleanenv: false` to drop `-e`.
...
`.cosigt.mk` by `make init` and `make check`:
