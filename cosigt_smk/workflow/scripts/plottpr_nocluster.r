...
for (r in region) {
    ...
    for (f in files[grepl(r,files)]) {
        ...
        if (grepl(sample, df$h1[1]) && grepl(sample, df$h2[1])) {

           #all_list[[f]]<-data.frame(region=r, sample=sample, value=1) 
           region_scores<-c(region_scores, 1)


        } else {

            print(sample)
            #all_list[[f]]<-data.frame(region=r, sample=sample, value=0) 
            region_scores<-c(region_scores, 0)
